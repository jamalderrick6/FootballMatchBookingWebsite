...
const ReservationCard = ({ reservation, onCancel }) => {
	const handleCancel = async () => {
		...
		try {
			const url = `reservations/${reservation.orderId}/delete`;
		...
		<div className="w-md-50 p-3 shadow-lg rounded-4 mb-3">
			...
			<Row className="mb-2 mb-md-3 justify-content-center">
				<Col className="d-flex flex-column justify-content-center align-items-center text-center">
					...
					<h5 className="fw-bold mt-2">
						OrderId :
						<span className="text-primary ms-2" style={{wordBreak:'break-all'}}>{reservation.orderId}</span>
					</h5>
					<h5 className="fw-bold mt-2">
						Seat Price :
						<span className="text-primary ms-2">
							Kshs. {reservation?.match?.seatPrice} 
						</span>
					</h5>
					<h5 className="fw-bold mt-2">
						Stadium :
